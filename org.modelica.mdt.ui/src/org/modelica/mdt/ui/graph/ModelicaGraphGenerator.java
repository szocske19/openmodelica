...
public class ModelicaGraphGenerator {
	...
	public static Map<String, ModelicaNode> nodesMap = new HashMap<String, ModelicaNode>();
	public static ModelicaTreeNode tree;
	...
	public static String makeGraph(){
		...
		graph += "fontsize = 36,";
		graph += "label = \"Tree of "   + nodes.get(0).className +  "\",";
		graph += "size = \"120,120\" ];";
		...
		for (ModelicaNode node : nodes)
		{
			graph += "\"" + node.getName() + "\"[color=" + getColorString(node.getColor()) +"];\n";
		}
		for (ModelicaConnection modelicaConnection : connections)
		{
			graph += "\"" + modelicaConnection.source.getName() + "\" -> \"" + modelicaConnection.destination.getName() + "\";\n";

		}
		graph += "}";
		System.out.println("Number of nodes: " + nodes.size());
		List<ModelicaNode> dontHaveChild = new ArrayList();
		for (ModelicaNode node : nodes)
		{
			dontHaveChild.add(node);
		}
		for (ModelicaConnection modelicaConnection : connections)
		{
			if(dontHaveChild.contains(modelicaConnection.source)){
				dontHaveChild.remove(dontHaveChild.indexOf(modelicaConnection.source));
			}

		}
		System.out.println("Number of leaf: " + dontHaveChild.size());
